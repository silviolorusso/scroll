...
function start() {
    if (status == 0) {
        ...
        if (!(jQuery.browser.mozilla)) {  jQuery(document).fullScreen(true); };
...
function main() {
    if (! jQuery('#style-scroll').length) {
        ...
        if (!(jQuery.browser.mozilla)) { 
             jQuery('<div id="blackscreen"><div id="title-scroll" class="title-scroll">ScrollTV<br/><span class="text-small-scroll">Click to play</span></div></div>').appendTo('body');
        } else {
